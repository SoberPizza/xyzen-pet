...
// ===== Utility tools =====
type EnvMode = 'off' | 'skyBox' | 'hemisphere'
...
interface IblUniforms {
  uSHCoeffs?: { value: THREE.Vector3[] }
  uNprEnvMode?: { value: number }
  uEnvIntensity?: { value: number }
}

function assignSHUniform(u: IblUniforms | undefined, sh: THREE.SphericalHarmonics3 | null | undefined) {
...
export function updateNprShaderSetting(
  ...
  root.traverse((o) => {
    ...
    mats.forEach((m) => {
      const u = (m?.userData as { __airiIbl?: IblUniforms } | undefined)?.__airiIbl
      if (!u?.uNprEnvMode || !u.uEnvIntensity)
        return
