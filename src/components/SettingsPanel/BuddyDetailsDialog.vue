...
const { buddies } = storeToRefs(buddyStore);

const buddy = computed(() =>
  props.buddyId ? buddies.value[props.buddyId] : undefined,
);
...
async function save() {
  if (!props.buddyId || !buddy.value) return;
  if (wakeWordsInvalid.value) {
    saveError.value =
      nameError.value ??
      nicknameErrors.value.find((e) => e !== null) ??
      WAKE_TERM_HINT;
    return;
  }
  saving.value = true;
  ...
  const existing = buddy.value;
  const patch: Partial<Buddy> = {};

  // --- Core fields ---
  if (form.name.trim() !== existing.name) patch.name = form.name.trim();

  const nicknames = form.nicknamesText
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
  const existingNicknames = existing.nicknames ?? [];
  const sameNicknames =
    nicknames.length === existingNicknames.length &&
    nicknames.every((n, i) => n === existingNicknames[i]);
  if (!sameNicknames) patch.nicknames = nicknames;

  const nextAvatar = form.avatar.trim() || undefined;
  if (nextAvatar !== existing.avatar) patch.avatar = nextAvatar;

  if (view.canEditGender && form.gender !== existing.gender)
    patch.gender = form.gender;

  // --- Advanced (dev-only) ---
  if (view.showAdvanced) {
    if (form.raceCode !== existing.raceCode) patch.raceCode = form.raceCode;
    if (form.stage !== existing.stage) patch.stage = form.stage;
    if (form.level !== existing.level) patch.level = form.level;
    if (form.xp !== existing.xp) patch.xp = form.xp;
    if (form.totalInteractions !== existing.totalInteractions)
      patch.totalInteractions = form.totalInteractions;

    const attrs = parseJson<Record<string, unknown>>(form.attributesText, {});
    if (!attrs.ok) {
      saveError.value = `Invalid attributes JSON: ${attrs.error}`;
      saving.value = false;
      return;
    }
    if (JSON.stringify(attrs.value) !== JSON.stringify(existing.attributes))
      patch.attributes = attrs.value;

    const generics = parseJson<string[]>(form.genericTraitCodesText, []);
    if (!generics.ok) {
      saveError.value = `Invalid generic_trait_codes JSON: ${generics.error}`;
      saving.value = false;
      return;
    }
    if (
      JSON.stringify(generics.value) !==
      JSON.stringify(existing.genericTraitCodes)
    )
      patch.genericTraitCodes = generics.value;
  }
  ...
  try {
    await buddyStore.updateBuddy(props.buddyId, patch);
    open.value = false;
  } catch (err) {
...
async function setActive() {
  if (!props.buddyId || isActive.value || activating.value) return;
  const targetId = props.buddyId;
  console.info('[buddy:activate] request', { buddyId: targetId });
  activating.value = true;
  saveError.value = null;
  try {
    await buddyStore.activateBuddy(targetId);
    console.info('[buddy:activate] store updated', { buddyId: targetId });

    // The active buddy drives both the stage VRM (via `useSettingsStageModel`
    // watching activeBuddyId) and the CEO topic binding (via `useCeoChatStore`
    // watching activeBuddyId). The watchers fire automatically, but:
    //   - `updateStageModel()` is idempotent and paints the correct VRM now
    //     in case the watcher hasn't flushed yet (same tick as the mutation).
    //   - SSE reconnect forces the backend-bound session/topic context to
    //     reinitialize against the new active buddy, so streamed
    //     emotion/gesture/text events land on the right topic immediately.
    stageModelStore.updateStageModel();
    console.info('[buddy:activate] stage model refreshed');

    try {
      const { sse } = await initXyzen();
      sse.reconnect();
      console.info('[buddy:activate] SSE reconnect triggered');
    } catch (err) {
      console.warn('[buddy:activate] SSE reconnect failed', err);
    }
  } catch (err) {
    console.error('[buddy:activate] failed', err);
    saveError.value = err instanceof Error ? err.message : String(err);
  } finally {
