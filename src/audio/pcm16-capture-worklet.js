 ...
 *
 * Kept as plain JS so Vite's `?worker&url` can serve it without a
 * TypeScript worklet pipeline.
 */
...
class Pcm16CaptureProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this._frameBuffer = new Float32Array(FRAME_SAMPLES)
    this._frameFill = 0
  }
  ...
  process(inputs) {
    ...
    for (let i = 0; i < mono.length; i++) {
      const cur = mono[i]
      // Emit every output sample whose time t lies in [i-1, i).
      while (cursor < 1) {
